...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    dashboardApi.getDashboard(month, year)
      ...
      .catch(() => toast.error(t('Erreur lors du chargement du tableau de bord')))
  ...
  return (
    <div className="space-y-6">
      <section className="hero-card overflow-hidden">
        ...
        <div className="grid gap-6 lg:grid-cols-[1.6fr_1fr] items-center">
          <div className="space-y-5">
            <p className="text-sm uppercase tracking-[0.3em] text-sky-300">{t('Tableau de bord')}</p>
            <h1 className="text-4xl font-bold leading-tight text-white">{t('Maîtrisez vos finances avec des indicateurs clairs et un design inspiré.')}</h1>
            <p className="max-w-2xl text-slate-300">{t('Visualisez vos revenus, dépenses, budgets et objectifs dans un environnement sombre, moderne et fluide.')}</p>
          </div>
          <div className="grid gap-4">
            <Card className="bg-white/5 border-white/10 p-6">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-400">{t('Solde actuel')}</p>
              <p className="mt-4 text-3xl font-semibold text-white">{formatCurrency(summary.netBalance)}</p>
              <p className="mt-2 text-sm text-slate-300">{t('Solde global disponible ce mois.')}</p>
            </Card>
            <Card className="bg-white/5 border-white/10 p-6">
              <p className="text-sm uppercase tracking-[0.2em] text-slate-400">{t('Revenus mensuels')}</p>
              <p className="mt-4 text-3xl font-semibold text-white">{formatCurrency(summary.totalIncome)}</p>
              <p className="mt-2 text-sm text-slate-300">{t('Voir l\'évolution de vos revenus récents.')}</p>
            </Card>
          </div>
        </div>
      </section>
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white">{t('Vue rapide')}</h2>
          <p className="text-sm text-slate-400">{t('Sélectionnez la période pour mettre à jour vos analyses.')}</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {summaryCards.map(c => (
          <Card key={c.label} className="flex items-center gap-4 bg-white/5 border-white/10">
            ...
            <div>
              <p className="text-sm text-slate-300">{t(c.label)}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">{t('Revenus vs Dépenses (6 derniers mois)')}</h3>
          <div className="h-[320px]">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={(incomeVsExpensesLast6Months || []).map(d => ({ ...d, month: formatMonth(d.month) }))}>
                ...
                <Bar dataKey="income" name={t('Revenus')} fill="#22C55E" radius={[8, 8, 0, 0]} />
                <Bar dataKey="expenses" name={t('Dépenses')} fill="#EF4444" radius={[8, 8, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </Card>
        ...
        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">{t('Dépenses par catégorie')}</h3>
          <div className="h-[320px]">
            {expensesByCategory?.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    outerRadius={90}
                    innerRadius={45}
                    label={({ categoryName, percentage }) => `${t(categoryName)} (${Math.round(percentage)}%)`}
                  >
              ...
              <p className="text-sm text-slate-400 text-center py-24">{t('Aucune dépense enregistrée pour cette période.')}</p>
            )}
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">{t('État des budgets')}</h3>
          {budgetStatus?.length > 0 ? (
            <div className="space-y-4">
              {budgetStatus.map(b => (
                <div key={b.budget?.id || b.categoryName} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-slate-200">{t(b.categoryName)}</span>
            ...
            <p className="text-sm text-slate-400 text-center py-8">{t('Aucun budget défini pour cette période.')}</p>
          )}
        </Card>

        <Card>
          <h3 className="text-sm font-semibold text-white mb-4">{t('Objectifs d\'épargne actifs')}</h3>
          {goalsProgress?.length > 0 ? (
            <div className="space-y-4">
              {goalsProgress.map(g => (
                <div key={g.goalName} className="space-y-1">
                  <div className="flex justify-between text-sm">
                    <span className="font-medium text-slate-200">{t(g.goalName)}</span>
            ...
            <p className="text-sm text-slate-400 text-center py-8">{t('Aucun objectif d\'épargne actif.')}</p>
          )}
        </Card>
      </div>
      ...
      {activeAlerts?.length > 0 && (
        <Card className="border-amber-200/30 bg-amber-500/10">
          <h3 className="text-sm font-semibold text-amber-200 mb-3 flex items-center gap-2">
            <AlertCircle size={16} /> {t('Alertes actives')}
          </h3>
          <div className="divide-y divide-white/10">
            {activeAlerts.map((a, i) => (
              <div key={i} className="py-2.5 first:pt-0 last:pb-0">
                <p className="text-sm font-medium text-white">{t(a.title)}</p>
                <p className="text-xs text-slate-400 mt-0.5">{t(a.message)}</p>
              </div>
            ))}
          </div>
        </Card>
