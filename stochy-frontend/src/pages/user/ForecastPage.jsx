...
export default function ForecastPage() {
  ...
  useEffect(() => {
    ...
    forecastApi.getForecast(months)
      ...
      .catch(() => toast.error('Erreur lors du calcul des prévisions'))
  ...
  const chartData = forecast.forecastMonths.map((m, idx) => ({
    name: m.month,
    'Solde projeté': m.projectedBalance
  }));
        ...
        <div className="h-[300px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={chartData}>
              ...
              <Line type="monotone" dataKey="Solde projeté" stroke="#2E5FA3" strokeWidth={3} dot={{ r: 4 }} activeDot={{ r: 6 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card className="border-blue-100 bg-blue-50/20">
          <h3 className="text-sm font-bold text-primary flex items-center gap-2 mb-4">
            <Sparkles size={16} /> Conseils et optimisations intelligents
          </h3>
          {forecast.suggestions?.length > 0 ? (
            ...
            <p className="text-sm text-gray-500">Aucune alerte ou conseil pour le moment. Votre situation financière semble saine et stable !</p>
          )}
        </Card>
        ...
        <Card className="border-red-100 bg-red-50/20">
          <h3 className="text-sm font-bold text-red-700 flex items-center gap-2 mb-4">
            <AlertTriangle size={16} /> Déficits ou anomalies de trésorerie détectés
          </h3>
            ...
            <div className="space-y-3">
              {forecast.forecastMonths.filter(m => m.isDeficit).map((m, idx) => (
                <div key={idx} className="flex gap-2.5 text-sm text-red-700 bg-red-50 p-2.5 rounded-lg">
                  ...
                  <span>Solde négatif projeté à {formatCurrency(m.projectedBalance)} !</span>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-sm text-emerald-700 font-medium">Aucun déficit de trésorerie n'est à prévoir sur cette période.</p>
          )}
        </Card>
      </div>
    </div>
