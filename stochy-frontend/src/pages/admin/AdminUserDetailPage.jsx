...
import { 
  ...
  MapPin, CheckCircle2, ShieldAlert, Edit2
} from 'lucide-react';
...
export default function AdminUserDetailPage() {
  ...
  const [editFormData, setEditFormData] = useState({
    firstName: '', lastName: '', email: '', phone: '', professionalStatus: '', gender: '', maritalStatus: ''
  });
  ...
  const handleEditSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await adminApi.updateUser(user.id, editFormData);
      setUser(res.data);
      setShowEditModal(false);
      toast.success("Détails de l'utilisateur mis à jour avec succès");
    } catch (err) {
      toast.error('Erreur lors de la mise à jour des détails');
      console.error(err);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 glass-panel p-6 rounded-2xl">
        <div className="flex items-center gap-4">
          ...
          <button 
            ...
            className="p-2.5 text-slate-300 hover:text-amber-500 hover:bg-amber-500/10 rounded-xl transition-all border border-white/10"
        ...
        <div className="space-y-6">
          <Card title="Détails du Profil" className="h-full">
            <div className="flex flex-col items-center pb-6 border-b border-white/10">
              <div className="w-20 h-20 bg-primary/5 rounded-full flex items-center justify-center text-primary text-3xl font-bold uppercase mb-3 border-2 border-primary/20 overflow-hidden">
                {user.profilePicUrl ? (
                  <img src={`http://localhost:8080${user.profilePicUrl}`} alt={`${user.firstName} ${user.lastName}`} className="w-full h-full object-cover" />
                ) : (
            ...
            <div className="space-y-4 pt-6">
              ...
              {user.address && (user.address.country || user.address.region || user.address.municipality) && (
                <div className="flex gap-3 text-sm text-slate-300 pt-2 border-t border-white/10">
                  <MapPin size={16} className="text-slate-400 flex-shrink-0 mt-0.5" />
                  <div>
                    <p className="font-medium text-slate-300 mb-1">Adresse :</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <Card title="Prêts en cours" className="flex flex-col">
              {loans.length > 0 ? (
                <div className="space-y-4">
                  {loans.map(loan => (
                    <div key={loan.id} className="p-3.5 bg-white/5 rounded-xl border border-white/10 space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold text-white">{loan.lenderName}</p>
                          <p className="text-xs text-slate-400">{LOAN_TYPES[loan.loanType] || loan.loanType}</p>
                        </div>
                        <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${loan.isActive ? 'bg-emerald-50 text-emerald-600' : 'bg-white/10 text-slate-300'}`}>
                      ...
                      <div>
                        <div className="flex justify-between text-xs text-slate-300 mb-1">
                          ...
                          <span className="font-semibold text-white">{Math.round(loan.progressPercent)}%</span>
                        </div>
                      ...
                      <div className="flex justify-between text-xs pt-1 border-t border-white/10 text-slate-300">
                        <span>Restant : <strong className="text-white">{loan.remainingCapital} {currency}</strong></span>
                        <span>Mensualité : <strong className="text-white">{loan.monthlyPayment} {currency}</strong></span>
                      </div>
                    </div>
                  ))}
                </div>
              ) : (
                <div className="text-center py-6 text-slate-400 text-sm">Aucun prêt en cours enregistré.</div>
              )}
            </Card>
            ...
            <Card title="Dettes actives" className="flex flex-col">
              {debts.length > 0 ? (
                <div className="space-y-4">
                  {debts.map(debt => (
                    <div key={debt.id} className="p-3.5 bg-white/5 rounded-xl border border-white/10 space-y-2">
                      <div className="flex justify-between items-start">
                        <div>
                          <p className="font-semibold text-white">{debt.debtorName}</p>
                          <p className="text-xs text-slate-400">Prêté le {debt.loanDate ? new Date(debt.loanDate).toLocaleDateString('fr-FR') : '-'}</p>
                        </div>
                      ...
                      <div className="flex justify-between text-xs pt-1 border-t border-white/10 text-slate-300">
                        <span>Montant initial : <strong className="text-white">{debt.amountLent} {currency}</strong></span>
                ...
                <div className="text-center py-6 text-slate-400 text-sm">Aucune dette en cours enregistrée.</div>
              )}
            </Card>
        ...
        <div className="lg:col-span-1">
          <Card title="Objectifs d'Épargne" className="h-full">
            {goals.length > 0 ? (
              <div className="space-y-4">
                {goals.map(goal => (
                  <div key={goal.id} className="p-3.5 bg-white/5 rounded-xl border border-white/10 space-y-2">
                    <div className="flex justify-between items-start">
                    ...
                    <div>
                      <div className="flex justify-between text-xs text-slate-300 mb-1">
                        ...
                        <span className="font-semibold text-white">{Math.round(goal.progressPercent)}%</span>
                      </div>
                    ...
                    <div className="flex justify-between text-[11px] pt-1 text-slate-300">
                      <span>Cumulé : <strong className="text-white">{goal.currentAmount} {currency}</strong></span>
                      <span>Cible : <strong className="text-white">{goal.targetAmount} {currency}</strong></span>
                    </div>
                  </div>
                ))}
              </div>
            ) : (
              <div className="text-center py-12 text-slate-400 text-sm">Aucun objectif d'épargne configuré.</div>
            )}
          </Card>
        </div>
            ...
            {transactions.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="w-full text-left text-sm border-collapse">
                  <thead>
                    <tr className="border-b border-white/10 text-slate-400 font-medium text-xs uppercase tracking-wider">
                  ...
                  <tbody className="divide-y divide-white/10">
                    {transactions.slice(0, 10).map(t => (
                      <tr key={t.id} className="hover:bg-white/5 transition-colors">
                        ...
                        <td className="py-3.5 text-slate-300">{t.categoryName || '-'}</td>
              ...
              <div className="text-center py-12 text-slate-400 text-sm">Aucune transaction récente à afficher.</div>
            )}
          </Card>
        </div>
      ...
      <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)} title="Modifier les détails de l'utilisateur">
        <form onSubmit={handleEditSubmit} className="space-y-4">
          ...
          <Select 
            ...
            value={editFormData.professionalStatus} 
            onChange={e => setEditFormData({...editFormData, professionalStatus: e.target.value})}
            options={[
              { value: '', label: 'Non spécifié' },
              ...Object.entries(PROFESSIONAL_STATUSES).map(([k, v]) => ({ value: k, label: v }))
            ]}
          />
          <div className="grid grid-cols-2 gap-4">
            <Select 
              ...
              options={[
                { value: '', label: 'Non spécifié' },
                ...Object.entries(GENDERS).map(([k, v]) => ({ value: k, label: v }))
              ]}
            />
            <Select 
              ...
              options={[
                { value: '', label: 'Non spécifié' },
                ...Object.entries(MARITAL_STATUSES).map(([k, v]) => ({ value: k, label: v }))
              ]}
            />
          </div>
